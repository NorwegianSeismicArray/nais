...
class NBeatsPhaseNetMetadata(NBeatsPhaseNet):
    def __init__(self, num_outputs=None, metadata_model=None, ph_kw=None):
        """Provides a wrapper for PhaseNet with a metadata output, eg., when learning back azimuth.
        ...
        Args:
            ...
            ph_kw (dict): Args. for PhaseNet. 
        """
        super(NBeatsPhaseNetMetadata, self).__init__(**ph_kw)
        if metadata_model is None:
            self.metadata_model = tf.keras.Sequential([tfl.Flatten(),
                                                   tfl.Dense(128, activation='relu'),
                                                   tfl.Dense(num_outputs)])
...
class DoublePhaseNet(PhaseNet):
    def __init__(self,
        ...
        Args:
            ...
            name (str, optional): model name. Defaults to 'PhaseNet'.
        """
        super(DoublePhaseNet, self).__init__(name=name)
        self.num_classes = num_classes
        self.initializer = initializer
        self.kernel_regularizer = kernel_regularizer
        self.dropout_rate = dropout_rate
    ...
    def phasenet(self, x, input_skips=None):
        ...
        for i in list(range(len(self.filters)))[::-1]:
            x = self._up_block(self.filters[i], self.kernelsizes[i], x)
            if input_skips:
                s = crop_and_concat(skips[i], input_skips[i])
            else:
                s = skips[i]
            x = crop_and_concat(x, s)
        ...
        if self.num_classes is not None:
            x = tfl.Conv1D(self.num_classes,
                           1,
                           padding="same")(x)
            network_1_output = tfl.Activation(self.output_activation, dtype='float32')(x)
        else:
            network_1_output = x
        
        return network_1_output, skips

    def build(self, input_shape):
        inputs = tf.keras.Input(shape=input_shape[1:])

        network_1_output, network_1_skips = self.phasenet(inputs, None)        
        
        input_network_2 = tfl.Multiply()([network_1_output, inputs])

        network_2_output, _ = self.phasenet(input_network_2, network_1_skips)
        
        output = tfl.Concatenate()([network_1_output, network_2_output])
        
        output = tfl.Conv1D(self.num_classes,
                           1,
                           padding="same")(x)
        output = tfl.Activation(self.output_activation, dtype='float32')(output)
...
class DoublePhaseNetMetadata(PhaseNet):
    def __init__(self, num_outputs=None, metadata_model=None, ph_kw=None):
        """Provides a wrapper for PhaseNet with a metadata output, eg., when learning back azimuth.
        ...
        Args:
            ...
            ph_kw (dict): Args. for PhaseNet. 
        """
        super(DoublePhaseNetMetadata, self).__init__(**ph_kw)
        if metadata_model is None:
            self.metadata_model = tf.keras.Sequential([tfl.Flatten(),
                                                   tfl.Dense(128, activation='relu'),
                                                   tfl.Dense(num_outputs)])
